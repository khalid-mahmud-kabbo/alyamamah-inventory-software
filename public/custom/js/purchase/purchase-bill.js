"use strict";
    ...
    function pageRedirect(formObject){
        ...
        if(formObject.response.id !== undefined){
            redirectTo = '/purchase/bill/details/'+formObject.response.id;
        }else{
            redirectTo = '/purchase/bill/list';
        }
        setTimeout(function() {
    ...
    */
    async function deletePaymentRequest(paymentId) {
        ...
        if (confirmed) {
            var url = baseURL + '/payment/purchase-bill/delete/';
    ...
    /**
     * is Shipping distribute accross the items or not
    */
    function isDistributeShippingChargeChecked() {
        return $('#is_shipping_charge_distributed').prop('checked');
    }
