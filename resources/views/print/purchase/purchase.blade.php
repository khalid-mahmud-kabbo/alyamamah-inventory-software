        ...
        <div class="invoice">
            <table class="header">
                <tr>
                    ...
                    <td class="bill-info">
                        <span class="bill-number">{{ __('print.bill') }} #: {{ $purchase->purchase_code }}</span><br>
                        <span class="cu-fs-16">{{ __('print.date') }}: {{ $purchase->formatted_purchase_date  }}</span><br>
                        <span class="cu-fs-16">{{ __('print.time') }}: {{ $purchase->format_created_time }}</span><br>
                        @if($purchase->reference_no)
                        <span class="cu-fs-16">{{ __('print.supplier_invoice_number') }}: {{ $purchase->reference_no  }}</span><br>
                        @endif
                        @if($purchase->purchase_order_id)
                        <span class="cu-fs-16">{{ __('print.order.order') }}: {{ $purchase->purchaseOrder->order_code  }}</span><br>
                        @endif
                    </td>
                </tr>
            </table>
            <table class="addresses">
                <tr>
                    <td class="address">
                        <span class="fw-bold cu-fs-18">{{ __('print.bill_from') }}</span><br>
                    ...
                    <td class="address">
                        <span class="fw-bold cu-fs-18">{{ __('print.ship_from') }}</span><br>
            ...
            <tfoot>
                ...
                <tr>
                    <td colspan="{{ $noteColumns }}" rowspan="{{$noteRowSpan}}" class="tfoot-first-td">
                        <span class="invoice-note">{{ __('print.note') }}:<br></span>{{ $purchase->note }}
                    </td>
                    <td colspan="2" class="text-end fw-bold">{{ __('print.subtotal') }}</td>
                    <td colspan="1" class="text-end">{{ $formatNumber->formatWithPrecision($subtotal) }}</td>
                </tr>
                <tr>
                    <td colspan="2" class="text-end fw-bold">{{ __('print.discount') }}</td>
                    <td colspan="1" class="text-end">{{ $formatNumber->formatWithPrecision($discount) }}</td>
                </tr>
                @if(app('company')['tax_type'] != 'no-tax')
                <tr>
                    <td colspan="2" class="text-end fw-bold">{{ __('print.tax') }}</td>
                    <td colspan="1" class="text-end">{{ $formatNumber->formatWithPrecision($taxAmount) }}</td>
                </tr>
                @endif
                @if($purchase->shipping_charge>0)
                <tr>
                    <td colspan="2" class="text-end fw-bold">{{ __('print.shipping_charge') }}</td>
                    <td colspan="1" class="text-end">{{ $formatNumber->formatWithPrecision($purchase->shipping_charge) }}</td>
                </tr>
                @endif
                <tr>
                    <td colspan="2" class="text-end fw-bold">{{ __('print.round_off') }}</td>
                ...
                <tr>
                    ...
                    <td colspan="2" class="text-end fw-bold">{{ __('print.grand_total') }}</td>
                ...
                @if(app('company')['is_enable_secondary_currency'])
                    <tr>
                        <td colspan="2" class="text-end fw-bold">{{ __('print.converted_to').'-'.$purchase->currency->code }}</td>
                        <td colspan="1" class="text-end">{{$formatNumber->formatWithPrecision($purchase->grand_total * $purchase->exchange_rate)}}</td>
                    </tr>
                @endif
                <tr>
                    <td colspan="2" class="text-end fw-bold">{{ __('print.paid_amount') }}</td>
                    <td colspan="1" class="text-end">{{$formatNumber->formatWithPrecision($purchase->paid_amount)}}</td>
                </tr>
                <tr>
                    <td colspan="2" class="text-end fw-bold">{{ __('print.balance') }}</td>
                ...
                <tr>
                    ...
                    @endphp
                    <td colspan="2" class="text-end fw-bold">{{ __('print.you_saved') }}</td>
                ...
                <tr>
                    ...
                    @endphp
                    <td colspan="2" class="text-end fw-bold">{{ __('print.total_due_balance') }}</td>
        ...
        <table class="table table-bordered custom-table tax-breakdown table-compact">
                 ...
                 @else
                    {{-- GST --}}
                     <tr>
                        ...
                        <th colspan="2" class="text-center">{{ __('print.cgst') }}</th>
                        <th colspan="2" class="text-center">{{ __('print.sgst') }}</th>
                        <th colspan="2" class="text-center">{{ __('print.igst') }}</th>
            ...
            <tbody>
                ...
                if(app('company')['tax_type'] == 'tax'){
                    ...
                    //GST
                    $taxSummary = $purchase->itemTransaction
                    ->groupBy('print.hsn') // First group by HSN
        ...
        <table class="addresses">
            <tr>
                <td class="address">
                    ...
                    <span class="cu-fs-14 ">{{ $purchase->party->note }}</span>
                </td>
            </tr>
        </table>
